...
int main(int argc,char *argv[])
{
  ...
  if (!status.batch && !quick && !opt_html && !opt_xml)
  {
    ...
    {
      char* str_env = getenv("HOME");
      size_t histfile_sz = (uint) strlen(str_env) + (uint) strlen("/.mysql_history") + 2;
      histfile=(char*) my_malloc(histfile_sz, MYF(MY_WME));
      if (histfile)
      {
        snprintf(histfile, histfile_sz, "%s/.mysql_history", str_env);
      }
	...
	tee_fprintf(stdout, "Reading history-file %s\n",histfile);
      ...
      size_t histfile_tmp_sz = (uint) strlen(histfile) + 5;
      if (!(histfile_tmp= (char*) my_malloc(histfile_tmp_sz, MYF(MY_WME))))
      {
